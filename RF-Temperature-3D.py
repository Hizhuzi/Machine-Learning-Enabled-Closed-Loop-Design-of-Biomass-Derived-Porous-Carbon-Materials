...
data = pd.read_excel("3D-synBDPCs.xlsx", usecols=[0, 1, 2, 3, 4, 9, 10, 11])
...
for fold, (train_index, test_index) in enumerate(kf.split(X), start=1):
    ...
    rf = RandomForestRegressor(criterion='squared_error',  # Change this parameter
                               n_estimators=340,
                               min_samples_split=2,
                               max_depth=12,
                               random_state=34)
    ...
    # Plot and display the scatter plot for the current fold in Spyder's plots window
    ...
    # Save the true and predicted values for each fold to Excel files
