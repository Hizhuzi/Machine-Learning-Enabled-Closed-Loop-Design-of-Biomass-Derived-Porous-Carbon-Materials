...
# Load the dataset
data = pd.read_excel("2D-synBDPCs.xlsx", usecols=[0, 8, 9])
...
dump(rf, 'random_forest_model-2D-Pre-carbonization.joblib')
...
def plot_roc_curve(y_true, y_pred_proba_dict, n_classes, labels, title='ROC Curve'):
    ...
    for i in range(n_classes):
        fpr, tpr, _ = roc_curve(y_true[:, i], y_pred_proba_dict[i])
        auc = roc_auc_score(y_true[:, i], y_pred_proba_dict[i])
