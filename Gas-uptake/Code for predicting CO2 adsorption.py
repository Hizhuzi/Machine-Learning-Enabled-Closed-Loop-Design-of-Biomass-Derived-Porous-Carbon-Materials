...
# Load datasets
df = pd.read_csv(r'./CO2-ULTRA.csv').drop(['No'], axis=1)
...
for rs in [7, 14, 21, 35, 42, 84, 100, 120, 200, 420]:
    ...
    with open('01 CO2 Retrain Scaler_%d.pickle' % rs, 'wb') as f:
        pickle.dump(sds, f)
    # PCA check
    pca = PCA(n_components=2)
    X_train_pca = pca.fit_transform(X_train_scaled)
    X_test_pca = pca.fit_transform(X_test_scaled)
    with open('01 CO2 Retrain PCA_%d.pickle' % rs, 'wb') as f:
        pickle.dump(pca, f)
    ## Visualize PCA
    '''plt.figure(figsize=(8, 8))
    ...
    plt.savefig('01 CO2 Retrain F5 PCA Visualization RS{}.png'.format(rs), dpi=300)
    plt.show()'''
    ...
    train_array.to_excel('01 CO2 Retrain RS{} train.xlsx'.format(rs))
    test_array.to_excel('01 CO2 Retrain RS{} test.xlsx'.format(rs))
    ...
    plt.scatter(y_test, y_pred_test, c='Tab:blue', alpha=0.6, edgecolors='none',
                label="Test (RMSE:{} R^2:{} MAE:{} MAPE:{})".format(df2.iloc[0, 1], df2.iloc[1, 1], df2.iloc[2, 1],
                                                                    df2.iloc[3, 1]))
    plt.plot([0, 10], [0, 10], c='orange', ls='--')
    plt.axis([0, 10, 0, 10])
    plt.xlabel('y_test', fontsize=18)
    plt.ylabel('y_pred', fontsize=18)
    plt.title('01 CO2 RS{}'.format(rs), fontsize=22)
    plt.legend(loc="lower right", frameon=False)
    plt.savefig('01 CO2 Retrain RS{}.png'.format(rs), dpi=300)
    plt.show()
    ...
    plt.legend()
    plt.savefig('01 CO2 Retrain Loss RS{}'.format(rs), dpi=300)
    plt.show()

    ch4_loss = pd.DataFrame(loss)
    ch4_loss.to_excel('01 CO2 Retrain Loss RS{}.xlsx'.format(rs))
