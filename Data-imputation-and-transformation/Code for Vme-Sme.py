...
# Load datasets
df = pd.read_csv(r'./★☆transfoamation_data_upload.csv').drop(['No','DOI'], axis=1)

# Draw distplot of uptake amount
plt.figure(figsize=(8, 8))
plt.xlabel('CO2 Uptake', fontsize=18)
plt.ylabel('Percentage', fontsize=18)
sns.distplot(df['SME'], axlabel='SME', label='cm^2')
plt.savefig('01 SME Retrain F1 Distplot.png', dpi=300)
plt.show()

# Draw pairplot and heatmap
plt.figure(figsize=(8, 8))
sns.pairplot(df.drop(['SMI'], axis=1), palette='husl', plot_kws=dict(edgecolor=None))
plt.savefig('01 Votal porevolume Retrain F2 Pairplot.png', dpi=300)
plt.show()

plt.figure(figsize=(10, 8))
ax = sns.heatmap(df.drop(['VTT'], axis=1).corr(), cmap="YlGnBu", annot=True, vmin=0, vmax=1)
ax.set_ylim([5, 0])
plt.savefig('01 Votal porevolume Retrain F3 Heatmap.png', dpi=300)
plt.show()

for rs in [7, 14, 21, 35, 42, 84, 100, 120, 200, 420]:
    # Split the datasets into training and test datasets
    X = df.drop(['SMI','SME','VTT'], axis=1)
    y = df['SME']
    ...
    with open('01 CH4 Retrain Scaler_%d.pickle' % rs, 'wb') as f:
        pickle.dump(sds, f)
    # PCA check
    pca = PCA(n_components=2)
    X_train_pca = pca.fit_transform(X_train_scaled)
    X_test_pca = pca.fit_transform(X_test_scaled)
    with open('01 CH4 Retrain PCA_%d.pickle' % rs, 'wb') as f:
        pickle.dump(pca, f)
    ## Visualize PCA
    '''plt.figure(figsize=(8, 8))
    ...
    '''def save_evaluate_draw(y_pred_train, y_pred_test, grid_param, best_params, best_estimator, ):
        ...
        test_array = pd.DataFrame({'y_test': np.array(y_test),
                                   ...
                                   'APE': np.abs((y_test xs- y_pred_test) / y_test) * 100})
        ...
        plt.scatter(y_train, y_pred_train, c='Tab:purple', alpha=0.6, edgecolors='none',
                    label="Train (RMSE:{} R^2:{} MAE:{} MAPE:{})".format(df2.iloc[0, 0], df2.iloc[1, 0], df2.iloc[2, 0],
                                                                         df2.iloc[3, 0]))
        plt.scatter(y_test, y_pred_test, c='Tab:blue', alpha=0.6, edgecolors='none',
                    label="Test (RMSE:{} R^2:{} MAE:{} MAPE:{})".format(df2.iloc[0, 1], df2.iloc[1, 1], df2.iloc[2, 1],
                                                                        df2.iloc[3, 1]))
    ...
    mlp = Sequential()
    mlp.add(Dense(units=64, activation='relu', input_dim=X_train_scaled.shape[1]))  # 修改处
    mlp.add(Dense(units=128, activation='relu'))
    mlp.add(Dense(units=32, activation='relu'))
    mlp.add(Dense(units=16, activation='relu'))
    mlp.add(Dense(units=1, activation='relu'))
    mlp.compile(optimizer='adam',
                loss='mean_squared_error',
                metrics=['mae'])
    # 二分类：激活函数sigmoid-损失函数：二元交叉熵
    # 多分类：激活函数：softmax-损失函数：多元交叉熵
    # 回归：激活函数：relu（或无），损失函数：均方差

    ch4_hist = mlp.fit(X_train_scaled, y_train, epochs=1000, batch_size=64)  # 修改处
    # Saving SSA model
    mlp.save('01 CH4_%d.h5' % rs)
    ...
    plt.scatter(y_test, y_pred_test, c='Tab:blue', alpha=0.6, edgecolors='none',
                label="Test (RMSE:{} R^2:{} MAE:{} MAPE:{})".format(df2.iloc[0, 1], df2.iloc[1, 1], df2.iloc[2, 1],
                                                                    df2.iloc[3, 1]))
    plt.plot([0, 3200], [0, 3200], c='orange', ls='--')
    plt.axis([0, 3200, 0, 3200])
    plt.xlabel('y_test', fontsize=18)
    plt.ylabel('y_pred', fontsize=18)
    plt.title('01 H2 RS{}'.format(rs), fontsize=22)
    plt.legend(loc="lower right", frameon=False)
