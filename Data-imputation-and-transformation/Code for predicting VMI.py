...
sns.distplot(df['VMI'], axlabel='VMI', label='cm^3')
plt.savefig('01 VMI Retrain F1 Distplot.png', dpi=300)
plt.show()

# Draw pairplot and heatmap
plt.figure(figsize=(8, 8))
sns.pairplot(df.drop(['VMI'], axis=1), palette='husl', plot_kws=dict(edgecolor=None))
plt.savefig('01 Votal porevolume Retrain F2 Pairplot.png', dpi=300)
plt.show()

plt.figure(figsize=(10, 8))
ax = sns.heatmap(df.drop(['VMI'], axis=1).corr(), cmap="YlGnBu", annot=True, vmin=0, vmax=1)
ax.set_ylim([2, 0])
