...
# Load datasets
df = pd.read_csv(r'./Data for predicting Capacitance.csv').drop(['No'], axis=1)

# Draw distplot of uptake amount
plt.figure(figsize=(8, 8))
plt.xlabel('Capacitance', fontsize=18)
plt.ylabel('Percentage', fontsize=18)
sns.distplot(df['Capacitance'], axlabel='Capacitance', label='F/g')
plt.savefig('01 CH4 Retrain F1 Distplot.png', dpi=300)
plt.show()

# Draw pairplot and heatmap
plt.figure(figsize=(8, 8))
sns.pairplot(df.drop(['DOI'], axis=1), palette='husl', plot_kws=dict(edgecolor=None))
plt.savefig('01 Supercapacitor Retrain F2 Pairplot.png', dpi=300)
plt.show()

plt.figure(figsize=(10, 8))
ax = sns.heatmap(df.drop(['DOI'], axis=1).corr(), cmap="YlGnBu", annot=True, vmin=0, vmax=1)
ax.set_ylim([5, 0])
plt.savefig('01 Supercapacitor Retrain F3 Heatmap.png', dpi=300)
plt.show()

for rs in [7, 14, 21, 35, 42, 84, 100, 120, 200, 420]:
    # Split the datasets into training and test datasets
    X = df.drop(['Capacitance', 'DOI'], axis=1)
    y = df['Capacitance']
    ...
    plt.scatter(y_test, y_pred_test, c='Tab:blue', alpha=0.6, edgecolors='none',
                label="Test (RMSE:{} R^2:{} MAE:{} MAPE:{})".format(df2.iloc[0, 1], df2.iloc[1, 1], df2.iloc[2, 1],
                                                                    df2.iloc[3, 1]))
    plt.plot([0, 700], [0, 700], c='orange', ls='--')
